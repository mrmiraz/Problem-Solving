...
public class Main {
    ...
    static public class boys{
        int bh, ba, bd;
        boys(int bh, int ba, int bd){
            this.bh = bh;
            this.ba = ba;
            this.bd = bd;
        }
    ...
    static int solver(){
        ...
        boys boy[] = new boys[X];
        for(int i =0; i < X; i++){
            int bh = sc.nextInt(), ba = sc.nextInt(), bd = sc.nextInt();
            boy[i] = new boys(bh, ba, bd);
        }
        ...
        for(int i =0; i < Y; i++){
            int gh = sc.nextInt(), ga = sc.nextInt(), gd = sc.nextInt();
            for(int j = 0; j < X; j++){
                int dh = Math.abs(boy[j].bh - gh);
                int da = Math.abs(boy[j].ba - ga);
                int dd = boy[j].bd - gd;
                if(dh <= 12 && da <= 5 && dd == 0){
                    graph[j].add(i);
                }
